...
RSpec.describe Skia::Shader do
  ...
  describe '.two_point_conical_gradient' do
    it 'raises UnsupportedOperationError when native API is unavailable' do
      ...
      allow(Skia::Native).to receive(:function_available?).with(:sk_shader_new_two_point_conical_gradient).and_return(false)

      expect do
        described_class.two_point_conical_gradient(
          Skia::Point.new(40, 40),
          10.0,
          Skia::Point.new(80, 80),
          60.0,
          [Skia::Color::RED, Skia::Color::BLUE]
        )
    ...
    it 'creates a two-point conical gradient shader' do
      shader = described_class.two_point_conical_gradient(
        Skia::Point.new(40, 40),
        10.0,
        Skia::Point.new(80, 80),
        60.0,
